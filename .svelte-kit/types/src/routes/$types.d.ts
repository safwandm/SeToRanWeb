...
type LayoutRouteId = RouteId | "/" | "/(layout)/dashboard" | "/(layout)/diskon" | "/(layout)/mitra" | "/(layout)/motor" | "/(layout)/pelanggan" | "/(layout)/pelanggan/[idPelanggan]" | "/(layout)/voucher" | "/(layout)/voucher/[voucherId]" | "/(noLayout)/login" | "/(noLayout)/register" | null
type LayoutParams = RouteParams & { idPelanggan?: string; voucherId?: string }
